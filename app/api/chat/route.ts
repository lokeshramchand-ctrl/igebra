...
export async function POST(request: Request) {
  try {
    ...
    if (queryEmbedding && queryEmbedding.length > 0) {
        const magnitude = Math.sqrt(queryEmbedding.reduce((sum: number, val: number) => sum + val * val, 0));
        queryEmbedding = queryEmbedding.map((val: number) => val / magnitude);
    }

    // 2. Search Supabase for the top 5 most relevant chunks
    const { data: chunks, error: searchError } = await supabaseAdmin.rpc('match_document_chunks', {
    ...
    if (!chunks || chunks.length === 0) {
      return NextResponse.json({ answer: "I couldn't find any information about that in the uploaded documents." });
    }
